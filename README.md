...
> **Repo layout:** the extension project lives in [`ableton-mcp/`](./ableton-mcp). Run all of
> the commands below from that directory. See [`ableton-mcp/README.md`](./ableton-mcp/README.md)
> for the in-project copy.
...
- **Ableton Extensions beta SDK + CLI** — `@ableton-extensions/sdk` and
  `@ableton-extensions/cli`. Not on npm; the beta tarballs are vendored in
  [`ableton-mcp/vendor/`](./ableton-mcp/vendor) and referenced via `file:vendor/*.tgz`. The CLI
...
```bash
cd ableton-mcp && npx vitest run    # unit tests for the pure logic (no live host needed)
```
